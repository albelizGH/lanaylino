...
const obtenerProductos = async (req, res) => {
    try{
        ...
        const [response,metadata] = await connection.query("select p.nombre as producto, p.descripcion as descripcion, p.precio as precio, p.genero as genero, p.imagen as ulrImagen, c.nombre as categoria from producto p join categoria c on p.id_categoria = c.id_categoria;")
...
const crearCategoria = async (req, res) => {
    try{
        ...
        const connection = await getConnection();
        const [response,metadata] = await connection.query("INSERT into categoria set ?", categoria);
        if(response && response.affectedRows > 0){
            res.json({
            codigo: 200,
